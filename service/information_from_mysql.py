    ...
    try:
        with connection.cursor() as cursor:
            if role == 'admin':
                ...
                sql = "SELECT session_id, chat_count, user_name, scenario, emotion, patient_details, conversation_score, performance_feedback FROM user_chat_history where publish_conversation =  1 or user_id =  %s"
            else:
                sql = "SELECT session_id, chat_count, user_name, scenario, emotion, patient_details, conversation_score, performance_feedback FROM user_chat_history where user_id =  %s"
