    ...
    try:
        with connection.cursor() as cursor:
            if role == 'admin':
                ...
                sql = "SELECT chat_count, user_name, patient_details, conversation_score, performance_feedback, session_id FROM user_chat_history where publish_conversation =  1 or user_id =  %s"
            else:
                sql = "SELECT chat_count, user_name, patient_details, conversation_score, performance_feedback, session_id FROM user_chat_history where user_id =  %s"
